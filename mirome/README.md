...
│           ├── perception.ts   Employee perception — three bands, never summed
│           └── actions.ts      First-30-days board + audience routing
...
| `mirome-brief.html` | **The company-facing half.** A board paper, not an instrument: the derived headline, three perception bands, where it is concentrated, the first-30-days action board with owner and cost on each, who should read what, and what the instrument cannot tell you. Generated by `scripts/build-brief.py` |
...
diagnosis. Regenerate after editing:

```bash
python3 scripts/split-prototype.py   # dashboard, from the combined source
python3 scripts/build-brief.py       # executive brief, from bundle.json
```

### The two halves of the company-facing side

The dashboard is **operated** — filtered, drilled into, assigned from. The
brief is **read**, once, by someone who will not touch a filter and will
forward it to someone else. Splitting them is what stops the CEO deck and the
HR working tool from ruining each other.

The brief answers four questions in order, and every number on it is derived:

1. **How do people see this company?** Three bands — *safety to speak*,
   *ability to deliver*, *ability to absorb change* — each with its own owner
   and its own fix. They are deliberately **never summed**. A single engagement
   score is what makes these platforms useless: it moves two points a year and
   nobody can act on it.
2. **Where is it concentrated?** Segments against the organisation, with
   suppressed groups shown as suppressed rather than silently dropped.
3. **What do we do first?** Three moves management can start inside 30 days
   with no budget and no facilitator, then the programme blocks. Each carries an
   owner, an approver, an effort and cost band, the expected index movement, the
   measure — and the plausible wrong move it displaces.
4. **Who should look at this?** Five roles, each scoped to a decision they can
   actually take, and each with an explicit *never sees*.
