...
**The interviewer is the surface, not a widget inside it.** She is one
persistent, full-bleed layer that is never unmounted and never scrolled away;
every other screen is something she hands you and takes back. Screens come in
two modes — *stage* (language, welcome, ready, the interview itself: content
floats on the studio falloff, no card) and *sheet* (consent, context, pulse,
reflection, confirmation, close: a white document that stops short of the top
so she stays visible above it). Her caption is alive on every screen, so the
participant is never handed a form by nobody, and her name card carries the
"AI, not a person" disclosure in a place it cannot be truncated out of.

The drawn figure is a **stand-in**, deliberately backlit and defocused rather
than a rendered face — a hand-drawn face lands in the uncanny valley and
cheapens the thing it is meant to carry. The composition is the deliverable:
point `DH_IMAGE` at a still of the production avatar (or a `data:` URI, so the
page stays self-contained) and the drawing is replaced without another line
changing anywhere on the page.
